...
int main( int argc, char *argv[] )
{
    ...
    for( i = 0; i < num_unlocked_cards; i++ )
    {
        ...
        else
        {
            ...
            gpsdo_status = skiq_is_gpsdo_supported( unlocked_cards[i], &gpsdo_support );
            ...
            case skiq_gpsdo_support_is_supported:
                printf("available and present\n");
                break;
            case skiq_gpsdo_support_card_not_supported:
                printf("not supported by card\n");
                break;
            case skiq_gpsdo_support_fpga_not_supported:
                printf("loaded FPGA bitstream does not implement functionality\n");
                break;
            ...
            default:
                printf("unknown status (%" PRIi32 ")\n", gpsdo_status);
                break;
            }

            if ( param.fpga_param.fpga_state == FPGA_STATE_VERSION_INACCESSIBLE )
            {
                printf("  FPGA\n"
                    "\tINACCESSIBLE BITSTREAM\n"
                    "\tversion: not available\n"
                    "\tgit hash: not available\n"
                    "\tbuild date (yymmddhh): not available\n");
            }
            else
            {
                printf("  FPGA\n"
            ...
            if(param.fpga_param.baseline_hash != 0)
            {
                printf("\tbaseline git hash: 0x%08x\n", param.fpga_param.baseline_hash);
            }
            ...
            if( !do_full_init )
            {
                ...
                if( skiq_read_calibration_date(unlocked_cards[i],
                                               ...
                                               &cal_interval) == 0 )
                {
                    printf("\tlast factory calibration year: %u\n"
                           "\tlast factory calibration week number: %u\n"
                           "\trecalibration interval: %u year(s)\n",
            ...
            else
            {
                ...
                {
                    skiq_rx_param_t *p_rx_param = &(param.rx_param[j]);
                    skiq_rx_hdl_t rx_hdl = param.rf_param.rx_handles[j];

                    printf("  RX[%d]: %s\n"
                           "\tLO tuning: %" PRIu64 " Hz - %" PRIu64 " Hz\n"
                           "\tsample rate: %" PRIu32 " Hz - %" PRIu32 " Hz\n",
                           j, rxHdlToString(rx_hdl),
                           p_rx_param->lo_freq_min,
                           p_rx_param->lo_freq_max,
                           p_rx_param->sample_rate_min,
                           p_rx_param->sample_rate_max);
                    printf("\tfilters: %d\n", p_rx_param->num_filters);
                    for( k = 0; k < p_rx_param->num_filters; k++ )
                    {
                        printf("\t\t- %s\n",
                               SKIQ_FILT_STRINGS[p_rx_param->filters[k]]);
                    }
                    if( p_rx_param->num_fixed_rf_ports > 0 )
                    {
                        printf("\tRX Fixed RF ports: %d\n", p_rx_param->num_fixed_rf_ports);
                        for( k = 0; k < p_rx_param->num_fixed_rf_ports; k++ )
                        {
                            printf("\t\t- %s",
                                   skiq_rf_port_string(p_rx_param->fixed_rf_ports[k]));
                            printf(" (cal data: %s)\n",
                                   rx_cal_data_cstr(unlocked_cards[i],
                                                    rx_hdl,
                                                    p_rx_param->fixed_rf_ports[k]));
                        }
                    }
                    if( p_rx_param->num_trx_rf_ports > 0 )
                    {
                        printf("\tRX TRX ports: %d\n", p_rx_param->num_trx_rf_ports);
                        for( k=0; k<p_rx_param->num_trx_rf_ports; k++ )
                        {
                            printf("\t\t - %s",
                                   skiq_rf_port_string(p_rx_param->trx_rf_ports[k]));
                            printf(" (cal data: %s)\n",
                                   rx_cal_data_cstr(unlocked_cards[i],
                                                    rx_hdl,
                                                    p_rx_param->trx_rf_ports[k]));
                        }
                    }
                    printf("\tRX calibration types available: \n");
                    if( p_rx_param->cal_type_mask != skiq_rx_cal_type_none )
                    {
                        if( (p_rx_param->cal_type_mask & skiq_rx_cal_type_dc_offset) != 0 )
                        {
                            printf("\t\t- DC offset\n");
                        }
                        if( (p_rx_param->cal_type_mask & skiq_rx_cal_type_quadrature) != 0 )
                        {
                            printf("\t\t- Quadrature\n");
                        }
                    }
                ...
                {
                    skiq_tx_hdl_t tx_hdl = param.rf_param.tx_handles[j];
                    skiq_tx_param_t *p_tx_param = &(param.tx_param[j]);

                    printf("  Tx[%d]: %s\n"
                           "\tLO tuning: %" PRIu64 " Hz - %" PRIu64 " Hz\n"
                           "\tsample rate: %" PRIu32 " Hz - %" PRIu32 " Hz\n"
                           "\tfilters: %d\n",
                           j, txHdlToString(tx_hdl),
                           p_tx_param->lo_freq_min,
                           p_tx_param->lo_freq_max,
                           p_tx_param->sample_rate_min,
                           p_tx_param->sample_rate_max,
                           p_tx_param->num_filters);
                    for( k = 0; k < p_tx_param->num_filters; k++ )
                    {
                        printf("\t\t- %s\n",
                               SKIQ_FILT_STRINGS[p_tx_param->filters[k]]);
                    }
                    if( p_tx_param->num_fixed_rf_ports > 0 )
                    {
                        printf("\tTX Fixed RF ports: %d\n", p_tx_param->num_fixed_rf_ports);
                        for( k = 0; k < p_tx_param->num_fixed_rf_ports; k++ )
                        {
                            printf("\t\t- %s\n",
                                   skiq_rf_port_string(p_tx_param->fixed_rf_ports[k]));
                        }
                    }
                    if( p_tx_param->num_trx_rf_ports > 0 )
                    {
                        printf("\tTX TRX RF ports: %d\n", p_tx_param->num_trx_rf_ports);
                        for( k = 0; k < p_tx_param->num_trx_rf_ports; k++ )
                        {
                            printf("\t\t- %s\n",
                                   skiq_rf_port_string(p_tx_param->trx_rf_ports[k]));
                        }
                    }
                }
            }
